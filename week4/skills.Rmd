...
* [Electric Vehicle Charging Stations](https://opendata.bristol.gov.uk/datasets/ebfdcdd3a6c848a4ac8190b2dde893ca_21/explore)
...
ev_charge <- here("week3",
             "examples",
             "Electric_vehicle_charging_points_NCR.csv") |>
  read_csv() |>
...
Here is how you would calculate the distance from each of a set of points to
a polygon that is the only feature in its layer.

```{r}
pumps <- pumps |>
  mutate(distance_to_aqma = (st_distance(pumps, aqma))) |>
  select(NAME, distance_to_aqma)
...
ggplot() +
  geom_sf(data = aqma) +
  geom_sf(data = pumps,
          aes(color = as.numeric(distance_to_aqma))) +
...
If you want to calculate the distance from a feature to the nearest feature in
another layer (one that includes more than one feature), you'll need to first
use the function `st_nearest` to identify the closest feature, then you can
find the distance to the nearest feature.

```{r}
pumps <- pumps |>
  mutate(nearest_ev = st_nearest_feature(pumps, ev_charge)) |>
  mutate(dist_to_ev = st_distance(pumps, 
                                  ev_charge[nearest_ev,], 
                                  by_element = TRUE)) 
         
 head(pumps) |>
  st_drop_geometry() |>
  select(NAME, nearest_ev, dist_to_ev) |>
  kable()        
```

```{r}
ggplot() +
    geom_sf(data = ev_charge,
          shape = 3,
          size = 1) +
    geom_sf(data = pumps,
          size = 3,
          aes(color = as.numeric(dist_to_ev))) +
  scale_color_viridis_c(trans = "log",
                        name = "Bike pumps by\nsistance from\nnearest EV charging\npoint (meters)",
                        breaks = breaks <- 100*2^seq(0, 20, by=1),
                        labels = prettyNum(breaks, big.mark = " ")) +
...
# Identifying the polygon containing each point in a layer

If you have a layer of points and a layer of polygons and you want to know which
polygon each point is in, you can do a spatial join. A spatial joins adds variables
from one layer to the features on another, based on the relative locations of 
features (rather than on the values of a variable).

## R

You can do a spatial join in R using the st_join function.
...
To do a spatial join in QGIS, open the "Vector" menu and select "Data Management
Tools," then "Join Attributes by Location." Select the layer you want to add
variables to in the "Join to features in" drop-down. Select the layer you want 
add variables from in the "By comparing to" drop-down. Click Run.
...
In ArcGIS, right-click in the contents pane on the layer you want to add
variables to. Select "Joins and Relates," then "Add Spatial Join."

Under "Target Features," select the layer you want to add variables to. Under
"Join Features." select the layer you want to add variables from. 

Under Match Option, "Intersect" will return variables from the features in your
join layer that overlap (intersect) with your target layer.

```{r, echo=FALSE, message=FALSE, out.width = '100%'}
here("week3",
     "examples",
     "gifs",
     "arc-spatial-join.gif") |>
  knitr::include_graphics()
```
